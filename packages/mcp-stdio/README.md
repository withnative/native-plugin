...
> **Unreleased candidate:** `0.1.0` is not yet available from npm. The commands below are
> release documentation and will fail until that exact package is published and accepted.
...
absolute path to override it. On macOS and Linux, directories are hardened to `0700` and
credential files to `0600`. The candidate does not establish or verify restrictive Windows
ACLs, so Windows credential storage is not supported for the `0.1.0` release gate.
